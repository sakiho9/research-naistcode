...
def cross_validate_and_evaluate(tokenizer, texts, labels, test_texts, test_labels, num_folds=5):
    ...
    results_file_path = os.path.join(results_dir, "all_results_jk.txt")
    ...
    with open(results_file_path, "w", encoding="utf-8") as result_file:
        for fold, (train_idx, val_idx) in tqdm(enumerate(skf.split(texts, labels)), total=num_folds, desc="Training Fold", ncols=100):
            ...
            model_dir = "/home/is/sakiho-k/research/notebooks/eating_disorder/multilingual/model/japanese_model"
            ...
            # Trainerのセットアップ
            training_args = TrainingArguments(
                output_dir=f"/home/is/sakiho-k/research/notebooks/eating_disorder/multilingual/model_japanese_korean/model_fold_{fold}",
                save_strategy="epoch",
                ...
                num_train_epochs=5,
                weight_decay=0.01,
                logging_dir=f"/home/is/sakiho-k/research/notebooks/eating_disorder/multilingual/model_japanese_korean/model_fold_{fold}/logs",
                save_total_limit=1,
...
def main():
    ...
    test_data_k = pd.concat([test_diet_k, test_disorder_k], axis=0)
    ...
    # 日本語の訓練データと検証データを分ける
    japanese_train_data = train_data_j.sample(frac=0.8, random_state=42)  # 訓練データ（80%）
    japanese_val_data = train_data_j.drop(japanese_train_data.index)  # 検証データ（20%）

    # 日本語モデルのファインチューニング
    japanese_train_dataset = preprocess_data(japanese_train_data, tokenizer)
    japanese_val_dataset = preprocess_data(japanese_val_data, tokenizer)

    # 日本語モデルの保存ディレクトリ
    japanese_model_dir = "/home/is/sakiho-k/research/notebooks/eating_disorder/multilingual/model/japanese_model"

    # ディレクトリが存在しない場合は作成
    os.makedirs(japanese_model_dir, exist_ok=True)
    # 日本語モデルのファインチューニング
    japanese_model, _ = train_model(japanese_train_dataset, japanese_val_dataset, japanese_model_dir)
    # ファインチューニング済みの日本語モデルを保存
    japanese_model.save_pretrained(japanese_model_dir)

    # 韓国語データでクロスバリデーションと評価
    cross_validate_and_evaluate(
        tokenizer=tokenizer,
        texts=train_data_k["text"].tolist(),
        labels=train_data_k["category"].tolist(),
        test_texts=test_data_k["text"].tolist(),
        test_labels=test_data_k["category"].tolist()
    )
