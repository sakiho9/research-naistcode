...
def scrape_ranking_items(url):
    ...
    try:
        with ThreadPoolExecutor(max_workers=10) as executor:
            futures = [executor.submit(scrape_blog_series, title, url, max_blog_pages) for title, url in zip(titles, urls)]
            for future in tqdm(futures, total=len(futures), desc="ブログ取得中"):
                items_data.extend(future.result())
...
# 実行
ranking_url = "https://blogger.ameba.jp/genres/diet/blogs/ranking"
